 ...
 */
 ...
 public:
      ...
      */
     explicit RawMemory(size_type capacity)
      ...
      */
     const_pointer operator+(size_t offset) const noexcept {
      ...
      */
     const_pointer GetAddress() const noexcept {
         return buffer_;
     }
     ...
     public:
          ...
          */
         static void Destroy(pointer begin, size_type count = 1) noexcept {
 ...
 private:
      ...
      */
     static void Deallocate(pointer buf) noexcept {
         operator delete(buf);
     }
 ...
 public:
     ...
     Vector(const Vector& other)
         : _data(other._size), _size(other._size) {
     ...
     void PushBack(_Ty&& value) {
         EmplaceOneAtBack(std::move(value));
     }
     ...
     void Reserve(size_type new_capacity) {
         if (new_capacity <= _data.Capacity()) return;
     ...
     iterator Insert(const_iterator pos, const _Ty& value) {
         return InsertOneOfThemByIndex(pos, value);
     }
      ...
      * @return Iterator to inserted element
      */
     iterator Insert(const_iterator pos, _Ty&& value) {
         return InsertOneOfThemByIndex(pos, std::move(value));
     }
 ...
 private:
     ...
     iterator InsertWithoutReallocation(const_iterator pos, Args&&... args) {
         ...
         try {
             ...
             if (index < _size) {
                 _Alloc_traits::ConstructRange(_data.GetAddress() + index, new_data.GetAddress() + index + 1, _size - index);
             }
         }
         catch (...) {
